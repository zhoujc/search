...
//    String strQuery = Tool.getInitialValue(request, "moviename","");
...
//	String strLicenseKey = Tool.getInitialValue(request, "key","");
	...
	boolean forshow = strforshow.equals("true")?true:false;
...
//	String usrQuery = "tag:10";//+" +tag:"+tag;
//	if(!tag.equals("10"))
//		usrQuery = usrQuery+" tag:"+tag;		
//    if(strCity.length()>0 ){
        try{
            //search
...
//				sr = POISearcher.searchNearby(strCity, querykey , poiType,  pt,                 nRange, strQuery ,iStart,iEnd, false);
				sr = POISearcher.search(strCity, querykey, poiType, usrQuery, iStart, iEnd, false, false,sortby,poiLon,poiLat);
//				sr = POISearcher.search("北京市", "图", "", "", 1, 30, false, false);
...
//		JSONObject p = new JSONObject();
		...
		if(poiStrLat.length()>0 && poiStrLon.length()>0&& sortby == 0)
			tempp.append("\"distance\":").append("\"").append(distance).append("\"").append(",");

		if(obj.getFieldValue("confidenceLevel").length()>0)
		...
		if(obj.getFieldValue("intro").length()>0)
			tempp.append("\"des\":").append("\"").append(obj.getFieldValue("intro")).append("\"").append(",");
